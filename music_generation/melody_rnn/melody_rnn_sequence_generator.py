...
class MelodyRnnSequenceGenerator(mm.BaseSequenceGenerator):
  ...
  def _generate(self, input_sequence, generator_options, return_loglik=False):
    ...
    generated_melody = self._model.generate_melody(
        end_step - melody.start_step, melody, return_loglik=return_loglik, **args)
    if return_loglik: 
      generated_melody, loglik = generated_melody

    generated_sequence = generated_melody.to_sequence(qpm=qpm)
    assert (generated_sequence.total_time - generate_section.end_time) <= 1e-5
    return generated_sequence, loglik
